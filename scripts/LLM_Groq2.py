...
class MultiRAGQueryAgent:
    ...
    def handle_query(self, user_input):
        ...
        # Extract keywords for background selection (regardless of query type)
        keywords = self.extract_general_darwin_keywords(user_input) if query_type > 1 else ""
        
        # Set up background clips based on query type and extracted keywords
        from PlaylistManagerTest import handle_background_selection
        handle_background_selection(query_type, keywords or user_input)
...
def generate_darwin_response(user_input):
    ...
    # First, handle the background selection based on query analysis
    # This is integrated inside the handle_query method
