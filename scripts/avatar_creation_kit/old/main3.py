# avatar_creation_kit.py - Avatar Creation Kit for Darwin AI (Fixed Avatar Dropdown & Folders)
...
def load_avatar_into_node_manager(avatar_name: str):
    ...
    try:
        ...
        print(f"[CREATION KIT] Both JSONs will save to: {avatar_dir}")
        return True
...
def save_character_creator_data():
    ...
    if not character_creator_data['name'] or character_creator_data['name'].strip() == "":
    ...
    try:
        ...
        if success:
            ...
            if NODE_MANAGER_AVAILABLE and character_creator_data['nodes']:
                ...
                if node_manager_instance:
                    ...
                    for i, char_node in enumerate(character_creator_data['nodes']):
...
def load_avatar_into_character_creator(avatar_name: str):
    ...
    if not avatar_name or avatar_name.strip() == "":
        print("[CREATION KIT] Warning: Empty avatar name provided for character creator")
        return False
...
def get_available_avatars():
    ...
    # Add default Darwin avatar
    if AVATAR_MANAGER_AVAILABLE:
        # Check if Darwin avatar exists in folder structure
        darwin_path = avatar_manager_instance.get_avatar_directory("Darwin")
        if (darwin_path / "avatar.json").exists():
            available_avatars.append("Darwin")
...
def create_main_ui():
        ...
        with ui.row().classes('w-full justify-between items-center'):
            ...
            with ui.row().classes('gap-4 items-center'):
                ...
                with ui.row().classes('gap-2 items-center'):
                    ...
                    avatar_select_component.on('update:model-value', lambda e: update_current_avatar(e.value if e.value else "Darwin"))
    ...
    def update_current_avatar(new_avatar_name):
        ...
        if not new_avatar_name or new_avatar_name.strip() == "":
            print("[CREATION KIT] Warning: Received empty avatar name, defaulting to Darwin")
            new_avatar_name = "Darwin"
        
        current_avatar_name = new_avatar_name
        print(f"[CREATION KIT] Updated current avatar to: '{current_avatar_name}'")
        
        # Update node manager to use new avatar directory
        if NODE_MANAGER_AVAILABLE and node_manager_instance:
            avatar_dir = get_current_avatar_directory()
            node_manager_instance.set_avatar_context(current_avatar_name, avatar_dir)
...
def create_character_generation_ui():
    """Create the Character Generation column UI - shortened for 900px height"""
    ...
    with ui.column().classes('w-full p-6 gap-4'):
        ...
        ).classes('w-full').props('outlined rows=10 filled')
