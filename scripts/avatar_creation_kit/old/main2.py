# avatar_creation_kit.py - Avatar Creation Kit for Darwin AI (Updated with Character Creator)
...
current_prompt = ""
current_page = "avatar_creation"  # Track current page: "avatar_creation", "supporting_assets", "node_manager", or "character_creator"
...
class CharacterNode:
    ...
    def __init__(self, name, prompt="", image_path=None):
        self.name = name
        self.prompt = prompt
    ...
    with ui.column().classes('w-full gap-4'):
        ...
        with ui.card().classes('w-full p-4'):
            ...
            def handle_portrait_upload_and_refresh(event):
                if event.content:
                    # Create a data URL for the image (in real app, you'd save the file)
                    import base64
        ...
        with ui.card().classes('w-full p-4'):
            ...
            with ui.card().classes('w-full p-4 bg-gray-50 mb-3'):
                ...
                def handle_node_image_upload_inline(event):
                    if event.content:
                        ui.notify(f"Image uploaded for new node")
                        # In a real implementation, you'd store this image temporarily
...
def create_character_creator_page():
    ...
    def finish_character_creation():
        # Here you would normally save the character data
        ui.notify(f"Character '{character_creator_data['name']}' created successfully!")
    ...
    with ui.column().classes('w-full min-h-screen bg-gray-100 flex items-center justify-center p-6'):
        ...
        def update_progress_indicators():
            # This would update the progress dots, but since they're created dynamically,
            # we'll handle this in the CSS animations instead
            pass
...
def clear_portrait_image():
    """Clear the portrait image"""
    global character_creator_data
    character_creator_data['portrait_image'] = None

def handle_portrait_upload(event):
    """Handle portrait image upload (legacy function - now handled inline)"""
    if event.content:
        import base64
        file_content = event.content.read()
        file_type = event.type or 'image/png'
...
def handle_node_image_upload(event, node_name_input):
    """Handle node image upload"""
    if event.content and node_name_input.value:
        # In a real implementation, you would save the file and store the path
        # For now, just show a notification
        ui.notify(f"Image uploaded for node")

def create_main_ui():
    """Create the main Creation Kit interface with page switching"""
    
    global current_page, main_update_page_content
...
def create_avatar_creation_page():
    """Create the Avatar Creation page with horizontal scrolling columns (Node Manager column removed)"""
