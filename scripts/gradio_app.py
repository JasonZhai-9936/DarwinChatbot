# Modified version of your app.py to work with Hugging Face Spaces
# Path: /data/DarwinChatbot/app.py
...
# Remaining functions (await_latentsync_output, idle_mode, response_mode, main_loop, 
# trigger_response_with_prompt, start_main_thread, shutdown) remain unchanged
...
# Custom UI builder that integrates with our system (unchanged)
def build_ui():
...
if __name__ in {"__main__", "__mp_main__"}:  # Support multiprocessing
    ...
    print(f"[SERVER] Starting NiceGUI on {host}:{port}")
    
    # Start the NiceGUI server with the host and port configured for Spaces
