...
def check_flag(step):
    return os.path.exists(os.path.join(FLAGS_DIR, f"{step}.flag"))

def set_flag(step):
    open(os.path.join(FLAGS_DIR, f"{step}.flag"), "w").close()
...
def install_other_requirements():
    ...
    if check_flag(step):
        print(f"⏩ Skipping {step} (already done)")
        return
    print("🔧 Installing other required packages (OpenCV)...")
    run([
        "conda", "run", "-n", CONDA_ENV,
